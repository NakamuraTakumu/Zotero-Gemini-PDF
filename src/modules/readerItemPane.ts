  ...
  static async registerReaderItemPaneSection() {
    Zotero.ItemPaneManager.registerSection({
      ...
      onInit: ({ body, refresh }) => {
        ...
        const handleGeminiAction = (event: Event) => {
          ...
          (async () => {
            ...
            const { managers, zoteroContext, chatData, runtimeState } = state; // parentItemFileMetadata を削除
            ...
            if (managers.chatManager && zoteroContext.actualParentItem && chatData.currentConversation && managers.uiManager) {
              ...
              if (!chatData.parentItemFileMetadata) {
                try {
                  ...
                  state.runtimeState.isGeminiRequestInProgress = false;
      ...
      onRender: async ({ body, item }) => {
        ...
        const paneState = addon.data.chatPanes[paneId];
        const { managers, uiElements, zoteroContext, chatData } = paneState;

        if (!managers.chatManager || !managers.uiManager) {
        ...
        if (actualParentItem) {
          try {
            ...
            paneState.chatData.currentConversation = currentConversation; // Use paneState.chatData.currentConversation
            managers.uiManager._renderChatMessages(paneState.chatData.currentConversation); // Use paneState.chatData.currentConversation
        ...
        paneState.chatData.parentItemFileMetadata = parentItemFileMetadata; // paneState.chatData.parentItemFileMetadataを格納
        ...
        const handleSendMessage = async () => {
          ...
          if (paneState.runtimeState.isGeminiRequestInProgress) {
            ...
            if (chatData.parentItemFileMetadata) {
              await managers.chatManager.processAndSendMessage(
                paneId, // Pass paneId
                messageText, messageText, zoteroContext.actualParentItem, chatData.currentConversation, // Changed currentConversation to chatData.currentConversation
                chatData.parentItemFileMetadata, // parentItemFileMetadata を渡す
        ...
        newChatButton.addEventListener("click", () => {
          ...
          const newConversation: ChatSessionHistory = {
            metadata: {
              zoteroParentItemKey: zoteroContext.actualParentItem.key,
              chatId: newChatId,
              chatTitle: newChatTitle,
            },
            history: [],
          };
          paneState.chatData.currentConversation = newConversation; // Use paneState.chatData.currentConversation
          managers.uiManager._renderChatMessages(paneState.chatData.currentConversation); // Clear and render empty chat
