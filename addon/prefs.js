pref("__prefsPrefix__.geminiApiKey", "");
pref("__prefsPrefix__.geminiSystemPrompt", "You are a helpful assistant that answers questions based on the provided context. Please provide your answer in markdown format, including Katex for mathematical expressions.");
pref("__prefsPrefix__.contextWindowSize", 32);
pref("__prefsPrefix__.promptForSelection", "As an academic assistant, please provide a concise explanation of the key concepts in the following selected text. Structure your answer clearly using markdown.\n\nSelected Text:\n---\n{selectedText}");
pref("__prefsPrefix__.geminiModelList", "gemini-2.5-flash-lite,gemini-2.5-flash,gemini-2.5-pro,gemini-3-pro-preview");
pref("__prefsPrefix__.geminiSelectedModel", "gemini-2.5-flash-lite");